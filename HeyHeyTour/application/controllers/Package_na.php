<?
    class Package_na extends CI_Controller {       
        function __construct()                 
        {
            parent::__construct();
            $this->load->database();           
            $this->load->model("package_na_m");
        ...
        public function lists()
        {
			...
			$config["per_page"]	 = 5;
			$config["total_rows"] = $this->package_na_m->rowcount($text1);
			...
			$data["list"]=$this->package_na_m->getlist($text1,$start,$limit);
		...
		public function view()
		{
			...
			$data["text1"]=$text1;
			$data["page"]=$page;
			$data["row"]=$this->package_na_m->getrow($no);
		...
		public function del()
		{
			...
			$this->package_na_m->deleterow($no);
		  	$uri_array=$this->uri->uri_to_assoc(3);
		    $no=array_key_exists("no",$uri_array) ? $uri_array["no"] : "" ;
		...
		{
			$data["list"] = $this->package_na_m->getlist_co();

			$this->load->view("main_header");
			$this->load->view("package_na_add", $data);    // 입력화면 표시
			$this->load->view("main_footer");
		}
		else              // 입력화면의 저장버튼 클릭한 경우
		{
			...
			$result = $this->package_na_m->insertrow($data); 
...
//			redirect("/~team13/package_as/lists/");
		...
		public function edit()
		{
			...
			if ($picname) $data["pic"]=$picname;
		        ...
		        $result = $this->package_na_m->updaterow($data, $no); 
				redirect("/~team13/package_na/lists" . $text1 . $page); 		
//				redirect("/~team13/package_as/lists/");
		    }
		}
