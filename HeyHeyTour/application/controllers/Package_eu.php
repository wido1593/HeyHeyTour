<?
    class Package_eu extends CI_Controller {       
        function __construct()                 
        {
            parent::__construct();
            $this->load->database();           
            $this->load->model("package_eu_m");
        ...
        public function lists()
        {
			...
			if ($text1=="") 
			    $base_url = "/package_eu/lists/page";
			else 
			    $base_url = "/package_eu/lists/text1/$text1/page";
			...
			$config["per_page"]	 = 5;
			$config["total_rows"] = $this->package_eu_m->rowcount($text1);
			...
			$data["list"]=$this->package_eu_m->getlist($text1,$start,$limit);

			$data["text1"]=$text1;
            $data["list"] = $this->package_eu_m->getlist($text1, $start, $limit); 

            $this->load->view("main_header");           
            $this->load->view("package_eu_list",$data);     
		...
		public function view()
		{
			...
			$data["text1"]=$text1;
			$data["page"]=$page;
			$data["row"]=$this->package_eu_m->getrow($no);

			$this->load->view("main_header");
            $this->load->view("package_eu_view",$data);
            $this->load->view("main_footer");
		}
		...
		public function del()
		{
			...
			$this->package_eu_m->deleterow($no);
		  	$uri_array=$this->uri->uri_to_assoc(3);
		    $no=array_key_exists("no",$uri_array) ? $uri_array["no"] : "" ;
		    $text1=array_key_exists("text1",$uri_array) ? "/text1/" . urldecode($uri_array["text1"]) : "";
			$page = array_key_exists("page", $uri_array) ?  "/page/" . urldecode($uri_array["page"]) : 0 ;		

			redirect("/~team13/package_eu/lists" . $text1 . $page); 
		}
		...
		{
			$data["list"] = $this->package_eu_m->getlist_co();

			$this->load->view("main_header");
			$this->load->view("package_eu_add", $data);    // 입력화면 표시
			$this->load->view("main_footer");
		}
		else              // 입력화면의 저장버튼 클릭한 경우
		{
			...
			$result = $this->package_eu_m->insertrow($data); 
...
//			redirect("/~team13/package_as/lists/");
		...
		public function edit()
		{
		    ...
		    if ($this->form_validation->run()==FALSE)
		    {
		        ...
		        $this->load->view("main_header");
		        $data["row"]=$this->package_eu_m->getrow($no);
		        $this->load->view("package_eu_edit",$data);
		        $this->load->view("main_footer");
		    }
		    else
		    {
			...
			if ($picname) $data["pic"]=$picname;
		        ...
		        $result = $this->package_eu_m->updaterow($data, $no); 
				redirect("/~team13/package_eu/lists" . $text1 . $page); 		
//				redirect("/~team13/package_as/lists/");
		    }
		}
