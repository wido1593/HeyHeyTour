<br><div role="alert"><h5><b>아시아</b></h5></div>
	<hr class="header">
<script>
	function find_text()
	{
		if(!form1.text1.value)
			form1.action="/~team13/package_asia/lists";
		else
			form1.action="/~team13/package_asia/lists/text1/" + form1.text1.value;
		form1.submit();
	}
</script>
...
?>
		...
		<a href="/~team13/package_asia/add<?=$tmp;?>" class="btn btn-sm mycolor1">추가</a>
...
<tr>
    <td><?=$no; ?></td>
    <td><a href="/~team13/package_asia/view/no/<?=$no?><?=$tmp?>"><?=$row->name;?></a></td>
    <td><?=$row->arr_time; ?></td>
    <td><?=$row->dep_time; ?></td>
    <td><?=$row->pic; ?></td>
