<br><div role="alert"><h5><b>유럽</b></h5></div>
	<hr class="header">
	<script>
	function find_text()
	{
		if(!form1.text1.value)
			form1.action="/~team13/package_eu/lists";
		else
			form1.action="/~team13/package_eu/lists/text1/" + form1.text1.value;
		form1.submit();
	}
</script>
...
?>
		...
		<a href="/~team13/package_eu/add<?=$tmp ;?>" class="btn btn-sm mycolor1">추가</a>
...
<tr>
    <td><?=$no; ?></td>
    <td><a href="/~team13/package_eu/view/no/<?=$no?><?=$tmp?>"><?=$row->name; ?></a></td>
    <td><?=$row->arr_time; ?></td>
    <td><?=$row->dep_time; ?></td>
    <td><?=$row->pic; ?></td>
